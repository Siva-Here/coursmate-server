...
const createResource = async (req, res) => {
  ...
  try {
    ...
    if (folder.name === "placements") {
      if (
        !(
          ...
          process.env.PLACEMENT_EMAILS.split(",").includes(email)
        )
      ) {
        return res
          .status(401)
          .json({ message: "Unauthorized to add a placement" });
      }
    }
    const authPlacement =
      process.env.ADMIN_EMAILS.split(",").includes(email) ||
    ...
    const newResource = new Resource({
      ...
      isAccepted:
        (folder.name === "placements" || folder.name === "Job Updates") &&
        authPlacement,
      isPlacement:
        (folder.name === "placements" || folder.name === "Job Updates") &&
        authPlacement,
  ...
  try {
    ...
    const updatedResources = await Promise.all(
      resources.map(async (resource) => {
        const user = await User.findById(resource.uploadedBy).select(
          "username"
        );
        return {
          ...resource._doc,
...
const createPost = async (req, res) => {
  const { name, description, posts, uploadedBy, parentFolder, rscLink } =
    req.body;
